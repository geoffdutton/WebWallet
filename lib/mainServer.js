    ...
    default:
      if (uri.split('/')[1].indexOf('.get') !== -1) DeliverFile()
        ...
        util.UpdateSession(sessionsDB, cookies['session'], {
          ...
          util.log('(Server.js:138) Cookies > ' + String(err), 1)
          DeliverFile()
        })
      }
      break
        ...
        if (resp.username === undefined || resp.username === '') {
          ...
          util.GetUser(usersDB, resp.username).then(resp => {
            if (resp.type >= 9) {  // When they have the right privileges deliver the actual site:
              // @TODO Move this to a method in admins.js model
              admins.adminExpire[admins.adminTokens.length] = Date.now() + config.Sessions.SessionTime * 60000
              admins.adminTokens[admins.adminTokens.length] = String(util.randtext(16, config.PassReq.Mask).replace('#', '!'))
              util.GetUser(usersDB, resp.username).then(resp => {
                DeliverPage(pages['AdminDash'].content.replace('//PrivateToken//', admins.adminTokens[admins.adminTokens.length - 1]), 200)
              }).catch(err => {
                util.log('Getting user: ' + String(err), 1)
              })
            } else {
